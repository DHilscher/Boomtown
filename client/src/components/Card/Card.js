...
export class SingleCard extends Component {
  render() {
    const { available, imageurl, itemowner, created, title, tags, description } = this.props.data;
    return (
      <Card>
        ...
        <CardActions>
          {available ? <RaisedButton onClick={available && false} backgroundColor='black' secondary={true} label="Borrow" /> : false}
        </CardActions>
      </Card>
    )
  }
